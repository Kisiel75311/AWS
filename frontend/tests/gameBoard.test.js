...
test.describe('GameBoard Component Tests', () => {
    test.beforeEach(async ({ page }) => {
        await page.goto('http://localhost:8080/'); // Adres URL Twojego projektu Vue.js
    });
    ...
    test('Should make a move and update the board', async ({ page }) => {
        await page.click('button:has-text("Start New Game")');
        // Wybierz pierwszą pustą komórkę i kliknij
        await page.click('.cell:has-text("")');
        // Sprawdź, czy pierwszy ruch został wykonany
        await expect(page.locator('.cell')).toContainText('X');
    });
